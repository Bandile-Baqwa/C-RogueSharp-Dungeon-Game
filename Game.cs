...
namespace RLNETConsoleGame
{
    public class Game
    {
        ...
        public static void Main()
        {
            ...
            Player.Item1 = new RevealMapDevice();   //the items the player will start the game with 
            Player.Item2 = new RevealMapDevice();
        ...
        private static void OnRootConsoleUpdate(object sender, UpdateEventArgs e)
        {
            ...
            else if (CommandSystem.IsPlayerTurn)
            {
                if (keyPress != null)
                {
                    ...
                    //    {
                    //        // add code thay will add item one forim the  shop inventory to the player iventory If statement for each 
                    //    }
                    //    if (keyPress.Key == RLKey.Number2)
                    //    {

                    //    }
                    //    if (keyPress.Key == RLKey.Number3)
                    //    {

                    //    }
                    //    if (keyPress.Key == RLKey.Number4)
                    //    {

                    //    }
                    //    if (keyPress.Key == RLKey.Number5)
                    //    {

                    //    }
                    //    if (keyPress.Key == RLKey.Number6)
                    //    {

                    //    }
                    //    if (keyPress.Key == RLKey.Number7)
                    //    {
                    ...
                    {
                        var hList = new Help();
                        hList.DisplayhelpList();
                    }
                    else if(keyPress.Key == RLKey.P)
                    {
                        DungeonMap pickUp = Game.DungeonMap;
                        Player player = Game.Player;

                        if (DungeonMap.IsPlayerOnTreasure(player, player.X, player.Y))
                        {
                            pickUp.PickUpTreasure(player, player.X, player.Y);
                        }
                    }
                    else
                    {
        ...
        private static void OnRootConsoleRender(object sender, UpdateEventArgs e)
        {
            if (_renderRequired)
            {
             ...
             DungeonMap.Draw(_mapConsole,_statsConsole,_inventoryConsole);
             
             Player.Draw(_mapConsole, DungeonMap);
             MessageLog.Draw(_messagesConsole);
                TargetingSystem.Draw(_mapConsole);
             //Player.DrawStats(_statsConsole);
             //PlayerInventory.DisplayPlayerInventory(_inventoryConsole);
